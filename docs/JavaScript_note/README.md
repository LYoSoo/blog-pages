...
简单基本类型 string 、 number 、 boolean 、 null 、 undefined  本身并不是对象，（typeof null 时会返回 object 时 BUG），null也是基本类型。

JavaScript 内置了一些对象子类型，也成为内置对象
...
这些内置函数可以当作构造函数使用，来构造对应的子对象类型的新对象。
